// client/src/WorkerPendingJob.jsx
...
// WorkerPendingJob component
function WorkerPendingJob() {
  ...
  useEffect(() => {
    ...
    if (!storedWorkerDetails) {
        ...
        const fetchJobs = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/jobs/pendingJobs/${worker._id}`, {
                    headers: {
  ...
  const fetchJobs = async () => {
    try {
      const jwtToken = sessionStorage.getItem('jwtToken');
      const response = await axios.get(`http://localhost:3001/jobs/pendingJobs/${workerId}`, {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
        },
      });
        ...
        <div className="col-md-6">
          <div className="card">
            <div className="card-body">
              <h3>Your Request Job Booking</h3>
            </div>
          </div>
          <div>
...
export default WorkerPendingJob;
