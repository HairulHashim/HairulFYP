// client/src/WorkerConfirmedJob.jsx
...
// WorkerConfirmedJob component
function WorkerConfirmedJob() {
  ...
  useEffect(() => {
    ...
    if (!storedWorkerDetails) {
        ...
        const fetchJobs = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/jobs/confirmedJobs/${worker._id}`, {
                    headers: {
  ...
  const fetchJobs = async () => {
    try {
      const jwtToken = sessionStorage.getItem('jwtToken');
      const response = await axios.get(`http://localhost:3001/jobs/confirmedJobs/${workerId}`, {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
        },
      });
              ...
              <div className="card" key={job._id}>
                <div className="card-body">
                  ...
                  <strong>Job Title:</strong> {job.title}<br />
                    ...
                    <strong>Clock In Time: </strong>{job.booked_worker.find(booking => booking.worker_id === workerId && booking.book_status === 'Confirmed').clock_in_time}<br />
                    <strong>Clock In Date: </strong>{job.booked_worker.find(booking => booking.worker_id === workerId && booking.book_status === 'Confirmed').clock_in_date}<br /><br />
...
export default WorkerConfirmedJob;
