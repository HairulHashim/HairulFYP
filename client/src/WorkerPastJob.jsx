...
// WorkerPastJob component
function WorkerPastJob() {
  ...
  useEffect(() => {
    ...
    if (!storedWorkerDetails) {
        ...
        const fetchJobs = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/jobs/pastJobs/${worker._id}`, {
                    headers: {
  ...
  const fetchJobs = async () => {
    try {
      const jwtToken = sessionStorage.getItem('jwtToken');
      const response = await axios.get(`http://localhost:3001/jobs/pastJobs/${workerId}`, {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
        },
      });
    ...
    <div style={{backgroundColor : "#f0f8ff",  minHeight: "100vh"}}>
      ...
      <div className="row justify-content-center">
        <div className="col-md-6">
          ...
          <div className="card">
            <div className="card-body">
                <h5><strong>Total Cancelled jobs:</strong> 1 jobs</h5>
                <h5><strong>Total Missing jobs:</strong> 0 jobs</h5>
            </div>
          </div><br></br>
          <ul>
              ...
              <div className="card" key={job._id}>
                <div className="card-body">
                  <li>
                    ...
                    <strong>Start Date:</strong> RM {job.job_rate}<br />
                    <strong>Cancelled Time: </strong>{job.booked_worker.find(booking => booking.worker_id === workerId && booking.book_status === 'Cancelled').cancel_time}<br />
                    <strong>Cancelled Reason: </strong>{job.booked_worker.find(booking => booking.worker_id === workerId && booking.book_status === 'Cancelled').cancel_reason}<br />
